...
// API RANDON USER FOR NAME OF USER
fetch(`https://randomuser.me/api/?results=1`).then(resposta=>{
    return resposta.json()
}).then(dados=>(dados.results[0].name.first))
.then(dados => {document.getElementById("userName").innerHTML=`${dados}`})
...
function addTodo(e) {
  ...
  if (todoInput.value.trim() === "") {
    openmodal("red", "Escreva uma tarefa antes!");
    return;
  }

  // alert("Duplicate task")
  if(isDuplicate(todoInput.value)){
    openmodal('red','Esta tarefa já existe!');
    return;
  }
...
function show_alert() {
  if (localStorage.getItem("todos") === null) {
    let html='Calma, voce precisa adicionar alguma tarefa primeiro';
    alert(html);
  } 
  else{
